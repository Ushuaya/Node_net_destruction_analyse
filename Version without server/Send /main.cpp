        ...
        if (in.is_open())
        {
            ...
            while (getline(in, line) && line != "}")
            {
                if(std::regex_search(line, m, regular_for_way))
                {
                    ...
                    {
                        cout << m.str(3);
                        mp[ m[3]] = quantity_of_nodes++;
                        g.emplace_back();
                    }
                    //Creating bind between two network buildings
                    g[mp[ m[1]]].push_back(mp[ m[3]]);
                    g[mp[ m[3]]].push_back(mp[ m[1]]);
            ...
            //if quantity of values smaller tham nodes defined in bondes part
            if (quantity_of_nodes != quantity_of_values)
                throw ("SYNTAX ERROR\n");
